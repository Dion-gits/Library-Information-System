...
class DeleteUserWindow:
    def setupUi(self, MainWindow):
        MainWindow.setObjectName("DeleteUserWindow")
        ...
        self.label_title = QtWidgets.QLabel("DELETE USER", parent=self.top_frame)
    ...
    def load_users(self, search_term=""):
        ...
        try:
            if search_term:
                ...
                users = self.db.fetch_all(query, (search_param, search_param, search_param))
            else:
    ...
    def select_user(self):
        ...
        from delete_confirmation_window import DeleteConfirmationWindow

        user_data = self.get_selected_user_data()
        if user_data:
            # Open the delete confirmation window with the selected user's data
            self.delete_confirmation_window = QtWidgets.QMainWindow()
            self.delete_confirmation_ui = DeleteConfirmationWindow()
            self.delete_confirmation_ui.setupUi(self.delete_confirmation_window, user_data)
            self.delete_confirmation_window.show()
        else:
            QtWidgets.QMessageBox.warning(None, "Selection Error", "Please select a user to delete!")
